<div
    class="col-span-1 max-w-xs rounded-2xl overflow-hidden shadow-lg bg-white transition-transform transform hover:scale-105">
    <a href="#">
        <img src="https://picsum.photos/150/100" alt="Product Image" class="w-full h-56 object-cover">
    </a>
    <div class="px-4 py-2">
        <div>
            <a href="#">
                <h3 class="font-bold text-lg text-red-500 hover:text-red-600 mb-1">Product Title</h3>
            </a>
            <p class="text-gray-700 text-sm mb-2">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Libero tempora dignissimos
                perferendis corrupti quis ut.
            </p>
        </div>
        <hr class="my-2">
        <div class="flex justify-between items-center">
            <p class="text-gray-900 font-bold text-md">$19.99</p>
            <button
                class="text-white bg-red-500 hover:bg-red-600 hover:text-black px-3 py-1 rounded-xl transition duration-300">
                Add to Cart
            </button>
        </div>
    </div>
</div>
